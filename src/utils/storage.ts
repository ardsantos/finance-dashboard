...
import { CategorizationRule } from "./categorizer";
...
const STORAGE_KEYS = {
  ...
  CATEGORIZATION_RULES: "finance_categorization_rules",
...
const validateCategories = (
  categories: any
): categories is TransactionCategories => {
  return (
    categories &&
    Array.isArray(categories.income) &&
    Array.isArray(categories.expenses) &&
    categories.income.every(
      (cat: any) =>
        typeof cat.id === "string" &&
        typeof cat.name === "string" &&
        typeof cat.icon === "string"
    ) &&
    categories.expenses.every(
      (cat: any) =>
        typeof cat.id === "string" &&
        typeof cat.name === "string" &&
        typeof cat.icon === "string"
...
export const saveCategorizationRules = (rules: CategorizationRule[]): void => {
  try {
    const serializedData = JSON.stringify(rules);
    safeSetItem(STORAGE_KEYS.CATEGORIZATION_RULES, serializedData);
    safeSetItem(STORAGE_KEYS.LAST_UPDATED, new Date().toISOString());
  } catch (error) {
    throw new StorageError(
      `Failed to save categorization rules: ${
        error instanceof Error ? error.message : "Unknown error"
      }`,
      error as Error
    );
...
export const loadCategorizationRules = (): CategorizationRule[] => {
  try {
    const data = safeGetItem(STORAGE_KEYS.CATEGORIZATION_RULES);
    if (!data) {
      return [];
    }
    ...
    if (!Array.isArray(parsed)) {
      throw new StorageError(
        "Invalid categorization rules data found in storage"
      );
    }
...
export const exportData = (): string => {
  try {
    ...
    const categorizationRules = loadCategorizationRules();
...
export const importData = (
  ...
  try {
    ...
    // Import categorization rules if present
    if (data.categorizationRules && Array.isArray(data.categorizationRules)) {
      saveCategorizationRules(data.categorizationRules);
      result.imported += 1;
    }
  ...
  for (let monthOffset = 2; monthOffset >= 0; monthOffset--) {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const transaction: Transaction = {
        ...
        description:
          brazilianDescriptions[
            Math.floor(Math.random() * brazilianDescriptions.length)
          ],
        amount,
        category,
        account:
          brazilianAccounts[
            Math.floor(Math.random() * brazilianAccounts.length)
          ],
...
// Utility function to clear only categorization rules (for testing/reset)
export const clearCategorizationRules = (): void => {
  try {
    localStorage.removeItem(STORAGE_KEYS.CATEGORIZATION_RULES);
    localStorage.removeItem(STORAGE_KEYS.LAST_UPDATED);
  } catch (error) {
    throw new StorageError(
      `Failed to clear categorization rules: ${
        error instanceof Error ? error.message : "Unknown error"
      }`,
      error as Error
    );
